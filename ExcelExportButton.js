	...
	var ExcelExportButton = Button.extend("ExcelExportButton", {
		metadata: {
			properties: {
				 ...
				 * <b>NOTE:</b> If <code>showSelectionDialog</code> is set to true, this property will determine
				 * if an Export All button will be displayed.
				 */
			...
			if (this._oList) {
				if (this.getShowSelectionDialog()) {
							...
							subHeader: [
								...
								buttons: [
									...
									new sap.m.Button(theExportButtonId2, {
										icon: "sap-icon://excel-attachment",
										text: (that.getExportAllRows()) ? "Export Visible Rows" : "Export",
										press: function(oEvent) {
											that._startExport(false);
										}
									}),
									new sap.m.Button({
										icon: that.getCloseButtonIcon(),
										text: that.getCloseButtonText(),
										press: function(oEvent) {
											that._closePDialog();
										}
									})
					...
					if (this.getExportAllRows()) {
						...
						this._oView.byId("exportLoadedButton").setText("Export (" + this._oList.getItems().length + ")");
					}
					this._pDialog.open();
					...
					if (this._oList.getModel().getProperty(sPath).length === this._oList.getItems().length) {
						this._oView.byId("exportAllButton").setVisible(false);
						this._oView.byId("exportLoadedButton").setText("Export (" + this._oList.getItems().length + ")");
					}
					else if (this.getExportAllRows()) {
						this._oView.byId("exportAllButton").setVisible(true);
					}
				}
				else {
					this._generateExcel();
